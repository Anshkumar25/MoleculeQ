...
def _scan_distances(r_min: float, r_max: float, n_points: int) -> list[float]:
    r_min = float(validate_bond_distance(r_min))
    r_max = float(validate_bond_distance(r_max))
    n = int(n_points)
    if n < 2:
    ...
    # Include both endpoints exactly; use log-spacing? Chemistry curves are
    # smooth in R; linear spacing is the honest default.
...
def run_scan(
    ...
    *,
    ...
    ----------
    r_min, r_max : float
        Scan window (angstrom).
    n_points : int
        Number of geometries, includes both endpoints.
    run_vqe_fn : optional callable
        ``fn(qubit_op, num_particles, nuclear_repulsion_energy) -> VQEResult``.
        Defaults to the bundled :func:`quantum_solver.run_vqe` with SLSQP at
        reduced cost (VQE is the expensive part of a scan).
    progress_callback : optional callable
        Invoked ``progress_callback(done, total)`` after each point so the
    ...
    ScanResult
    """
    distances = _scan_distances(r_min, r_max, n_points)
    ...
    if run_vqe_fn is None:
        ...
        def default_vqe(qubit_op, num_particles, enuc, *, maxiter=400):
            mapper = ParityMapper(num_particles=num_particles)
            return run_vqe(
                qubit_op,
                num_particles=num_particles,
                nuclear_repulsion_energy=enuc,
                ansatz="uccsd",  # chemically exact for 2e-/2-orbital; reliable across the curve
                optimizer="SLSQP",
                maxiter=maxiter,
                qubit_mapper=mapper,
